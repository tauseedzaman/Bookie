        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4 mb-6">
                ...
                @if ($favoriteBookmarks->isEmpty())
                    <p class="text-gray-500 dark:text-gray-400"><i class="fas fa-exclamation-circle"></i> No favorite
                        bookmarks yet.</p>
                @else
                    <ul class="list-none text-gray-800 dark:text-gray-200 space-y-4">
                        @php
                            // Filter out bookmarks without a category or with 0 bookmarks
                            $filteredBookmarks = $favoriteBookmarks->filter(
                                fn($bookmark) => $bookmark->category && $bookmark->category->bookmarks->count() > 0,
                            );

                            // Group filtered bookmarks by category
                            $groupedBookmarks = $filteredBookmarks->groupBy(
                                fn($bookmark) => $bookmark->category->name ?? 'Uncategorized',
                            );
                        @endphp
                                ...
                                <ul class="list-none text-gray-800 dark:text-gray-200 pl-4 space-y-2 mt-2">
                                    @foreach ($bookmarks as $bookmark)
                                        <li class="flex justify-between items-center border-b-2 pb-1 border-cyan-300">
                                            ...
                                            <!-- Unstar Button -->
                                            <button
                                                onclick="confirmUnfavorite({{ $bookmark->id }}, '{{ route('bookmark.unfavorite', $bookmark->id) }}')"
                                            ...
                                            <button
                                                onclick="confirmDelete({{ $bookmark->id }}, '{{ route('bookmark.destroy', $bookmark->id) }}')"
                                        ...
                                        @foreach ($category->bookmarks as $bookmark)
                                            <li class="flex justify-between items-center border-b border-b-gray-600">
                                                ...
                                                @else
                                                    ...
                                                    <button
                                                        onclick="confirmFavorite({{ $bookmark->id }}, '{{ route('bookmark.favorite', $bookmark->id) }}')"
                                                        class="text-yellow-500 hover:text-yellow-700">
